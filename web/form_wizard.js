...
formtowizard.prototype={
	...
	loadsection:function(rawi, bypasshooks){
  	...
  	//doload Boolean checks to see whether to load next section (true if bypasshooks param is true or onpagechangestart() event handler doesn't return false)
		...
		if (this.currentsection!=i && i<this.sections.count && doload){ //if next section to show isn't the same as the current section shown
			...
			this.paginatediv.$status.text("Paso "+(i+1)+" de "+this.sections.count) //update current page status text
	...
	validate:function(section){
		...
		var validated=true, invalidtext=["Debes completar los siguientes campos:\n"]
	...
	init:function(setting){
		...
		jQuery(function($){ //on document.ready
			...
			$sections.each(function(i){ //loop through $sections again
				...
				var $thestep=$('<div class="step disabledstep" />').data('section', i).html('Paso '+(i+1)+'<div class="smalltext">'+$section.find('legend:eq(0)').text()+'</div>').appendTo($stepsguide)
			...
			var $paginatediv=$('<div class="formpaginate" style="overflow:hidden;"><span class="prev" style="float:left">&laquo; Anterior</span> <span class="status">Paso 1 de </span> <span class="next" style="float:right">Siguiente &raquo;</span></div>')
